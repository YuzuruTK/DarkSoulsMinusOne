...
var enemy_type: Array[EnemyGenerator.EnemyType] = []
...
func _initialize_battle() -> void:
	camera.make_current()
	_setup_dependencies()
	if "alien" in battle_type:
		enemy_type.append(EnemyGenerator.EnemyType.ALIEN)
	if "skeleton" in battle_type:
		enemy_type.append(EnemyGenerator.EnemyType.SKELETON)
	if "wolf" in battle_type:
		enemy_type.append(EnemyGenerator.EnemyType.WOLF)
	if "bird" in battle_type:
		enemy_type.append(EnemyGenerator.EnemyType.BIRD)
	if "hog" in battle_type:
		enemy_type.append(EnemyGenerator.EnemyType.HOG)
	if "soldier" in battle_type:
		enemy_type.append(EnemyGenerator.EnemyType.SOLDIER)
	if "tiger" in battle_type:
		enemy_type.append(EnemyGenerator.EnemyType.TIGER)
	if "phantasm" in battle_type:
		enemy_type.append(EnemyGenerator.EnemyType.PHANTASM)
...
func _create_characters() -> void:
	...
	if enemy_type.is_empty():
		enemy_data = enemy_generator.generate_scaled_enemies(len(party_data), number_of_enemies)
	else:
		enemy_data = enemy_generator.generate_enemy_group(len(party_data) - 1, false, enemy_type)
...
func _is_battle_active() -> bool:
	return enemies.size() > 0 and party.filter(func(p: Player): return p.is_alive()).size() > 0
...
func _determine_battle_outcome() -> void:
	if enemies.size() == 0:
		...
		party.map(func(player:Player): player.actual_health = 1 if player.is_dead() else player.actual_health)
		_save_battle_results()
		onresult.emit("true")
