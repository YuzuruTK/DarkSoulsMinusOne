...
func show_current_question():
	if current_question < questions.size():
		question_label.text = "Sepé asks: \"" + questions[current_question] + "\""
		answer_input.text = ""
...
func end_game():
	...
	if success:
		...
		feedback_label.text = "\"You may pass in peace, friend. Your words show honor.\""
		feedback_label.modulate = Color.GREEN
	else:
		question_label.text = "Sepé raises his weapon..."
		feedback_label.text = "\"Your words do not convince me. Leave now!\""
		feedback_label.modulate = Color.RED
...
func restart_game():
	...
	submit_button.pressed.disconnect(restart_game)
	submit_button.pressed.connect(_on_submit_pressed)
	start_game()
